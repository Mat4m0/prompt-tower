...
export class PromptPresetApplicationService {
  ...
  async migrateOldPrefixHistory(): Promise<void> {
    ...
    const uniqueTexts = [
      ...new Set(oldHistory.map((entry) => entry.text).filter(Boolean)),
    ] as string[]
  ...
  getEffectivePrefix(): string {
    ...
    return activePreset ? getCurrentPromptPresetVersion(activePreset).text : this.getInlinePrefix()
  }
  ...
  async saveVersion(presetId: string, text: string, note?: string): Promise<PromptPreset> {
