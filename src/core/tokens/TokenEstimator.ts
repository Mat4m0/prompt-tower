...
export function formatTreeTokenCount(tokenCount: number, status: TreeTokenStatus): string {
...
export function recomputeTreeTokenCounts<TNode extends TreeTokenNode<TNode>>(node: TNode): void {
  if (!node.children) {
    node.displayTokenCount = node.exactTokenCount ?? node.estimatedTokenCount
    node.tokenCountStatus = node.exactTokenCount === undefined ? 'estimated' : 'exact'
    return
  }
...
export function updateLeafTreeTokenCounts<TNode extends TreeTokenNode<TNode>>(
  ...
  while (parent) {
    ...
    parent.tokenCountStatus =
      children.length > 0 && children.every((child) => child.tokenCountStatus === 'exact')
        ? 'exact'
        : 'estimated'
